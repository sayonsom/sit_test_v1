...
The screenshots in this revision are release-candidate evidence captured against the real frontend in a local browser with a deterministic, synthetic API fixture. They contain no live student records or client credentials. They prove rendering, interaction, computation, and staff-results presentation for the candidate code; they do **not** by themselves prove that the code has reached production. The production verification section must be completed after deployment against the canonical public route.
...
| [SIT-3](https://energyatlas.atlassian.net/browse/SIT-3) | Experiments are blank or unusable | Every experiment depended on dynamically injecting a compute script with one shared DOM ID. A stale script could be reused for a different experiment; load and compute failures were swallowed, leaving an indefinite spinner or empty panel. Several data-loading components also hid request failures. | Bundled five pure calculators with the application, removed runtime compute-script injection, validated finite series, and added loading/error/retry states across experiment, theory, course, and module loading. | Pass — 5/5 experiment forms |
...
**Status:** Pending deployment and authenticated verification.

The release is complete only after the merged production commit is deployed and these checks pass on the canonical route:
...
Production commit, deployment time, image/container identity, and live screenshots will be appended after deployment.
