...
class LTIRequestHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        """
        Allow GET for OIDC Login Initiation (/lti/login). Everything else returns 405.
        """
        try:
            logger.debug(f"Received GET request at path: {self.path}")
            parsed = urllib.parse.urlparse(self.path)
            if parsed.path == '/lti/login':
                # Parse query params and reuse the same handler
                query_params = urllib.parse.parse_qs(parsed.query)
                logger.debug(f"Parsed query params: {query_params}")
                handle_login(self, query_params)
            else:
